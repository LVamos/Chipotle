...
namespace Game.Terrain
{
	...
	public struct Rectangle
	{
		...
		/// <returns>A new point with coordinates clamped to the rectangle's boundaries.</returns>
		public Vector2 ClampPoint(Vector2 point)
		{
			float clampedX = Clamp(point.x, UpperLeftCorner.x, LowerRightCorner.x);
			float clampedY = Clamp(point.y, LowerRightCorner.y, UpperLeftCorner.y);
		...
		public static List<Vector2> GetPointsAround(Rectangle areaToAvoid, float minDistance, float maxDistance, float resolution, bool sameZone = true)
		{
			...
			IEnumerable<Vector2> candidatePoints = maxArea.GetPoints(resolution);
		...
		/// </returns>
		public Direction GetSide(Vector2 point)
		{
			Rectangle copy = this;
			return
			DirectionExtension.BasicDirections
				.FirstOrDefault(side => copy.LaysOnSide(side, point));
		}
		...
		public HashSet<Vector2> GetPointsByDistance(Vector2 point) => GetPoints().OrderBy(p => World.GetDistance(p, point)).ToHashSet();
		...
		public IEnumerable<Vector2> GetSurroundingPoints(int minDistance, int maxDistance, float tileSize = .1f)
		{
			...
			HashSet<Vector2> points = maxRectangle.GetPoints(tileSize);
...
.Where(point => !minRectangle.Contains(point));
		...
		public Vector2? GetAlignedPoint(Vector2 point, float distanceFromEdge = 0, bool clamp = false)
		{
			Rectangle copy = this;
			...
			// Pokud není rovnoběžný s žádnou hranou, vrať null
			...
			{
				if (point.y > MaxY)
					y = MaxY - distanceFromEdge;
				else if (point.y < MinY)
					y = MinY + distanceFromEdge;
			}
			else if (alignedHorizontally)
			{
				if (point.x < MinX)
					x = MinX + distanceFromEdge;
				else if (point.x > MaxX)
					x = MaxX - distanceFromEdge;
			}

			Vector2 result = new(x, y);
			return clamp ? copy.ClampPoint(result) : result;
		}
		...
		public HashSet<Vector2> GetPointsOutsideMap()
		{
			return GetPoints()
...
.ToHashSet();
		...
		{
			HashSet<Vector2> points = GetPoints(resolution);
