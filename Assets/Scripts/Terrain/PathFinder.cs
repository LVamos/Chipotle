...
Character character
		)
		{
			...
			Zone startZone = World.GetZone(start);
			Zone targetZone = World.GetZone(goal);
			...
			while (openQueue.Count > 0)
			{
				...
				List<PathFindingNode> neighbours = GetNeighbours(current, start, goal, sameZone, throughStart, throughGoal).ToList();
				foreach (PathFindingNode neighbour in neighbours)
				{
					...
					if (closed.Contains(neighbour.Coords)
						|| !IsWalkable(neighbour, start, goal, sameZone, throughStart, throughGoal, characterHeight, characterWidth, character)
						)
					{
...
Character character
		)
		{
			...
			bool noCharacters = !zone.Characters.Any(c => c != character && c.Area.Value.Contains(node.Coords));
