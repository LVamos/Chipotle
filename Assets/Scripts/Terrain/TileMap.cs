...
namespace Game.Terrain
{
    ...
    public class TileMap
    {
        ...
        public static Vector2 SnapToGrid(Vector2 point, float? tileSize = null)
        {
            ...
            float y = finalTileSize * Mathf.Round(point.y / finalTileSize);
        ...
        {
            HashSet<Vector2> points = area.GetPoints(TileSize);
            foreach (Vector2 point in points)
                PutTile(point, new(terrain, permeable, null));
        }

        private IEnumerable<Vector2> GetIntersectingTiles(Rectangle rect)
        {
            float minX = Mathf.Floor(rect.MinX / TileSize) * TileSize;
            float maxX = Mathf.Ceil(rect.MaxX / TileSize) * TileSize;

            float minY = Mathf.Floor(rect.MinY / TileSize) * TileSize;
            float maxY = Mathf.Ceil(rect.MaxY / TileSize) * TileSize;

            int minTileX = Mathf.FloorToInt(rect.MinX / TileSize);
            int maxTileX = Mathf.CeilToInt(rect.MaxX / TileSize);

            int minTileY = Mathf.FloorToInt(rect.MinY / TileSize);
            int maxTileY = Mathf.CeilToInt(rect.MaxY / TileSize);
            ...
            for (int tx = minTileX; tx <= maxTileX; tx++)
            {
                for (int ty = minTileY; ty <= maxTileY; ty++)
                {
                    ...
                    Rectangle tileRect = Rectangle.FromCenter(
                        new(x - half, y - half),
                        TileSize,
                        TileSize);
        ...
        private void DrawPanel(XElement panel, Rectangle zoneArea)
        {
            ...
            IEnumerable<Vector2> points = GetIntersectingTiles(panelArea);
            foreach (Vector2 point in points)
                PutTile(point, new(panelTerrain, permeable, null));
        }
