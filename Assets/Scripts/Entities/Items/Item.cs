...
namespace Game.Entities.Items
{
    ...
    public class Item : Entity
    {
        ...
        private const int _loopMaxDistance = 20;
        ...
        private bool IsInAudibleDistance()
        {
            float distanceToPlayer = _area.Value.GetDistanceFrom(World.Player.Area.Value);
            if (distanceToPlayer > _loopMaxDistance)
                return false;
            return true;
        }
        ...
        protected void UpdateOcclusion(ObstacleType obstacle = ObstacleType.None, float duration = _defaultOcclusionDuration)
        {
            ...
            if (_occludingObstacle == obstacle && obstacle != ObstacleType.None)
                return;
            ...
            switch (obstacle)
            {
                case ObstacleType.Wall:
                    cutoffFrequency = Sounds.OverWallLowpass;
                    volume = Sounds.GetOverWallVolume(_defaultVolume);
                    break;
                case ObstacleType.Door:
                    cutoffFrequency = Sounds.OverDoorLowpass;
                    volume = Sounds.GetOverDoorVolume(_defaultVolume);
                    break;
                case ObstacleType.ItemOrCharacter:
                    cutoffFrequency = Sounds.OverObjectLowpass;
                    volume = Sounds.GetOverObjectVolume(_defaultVolume);
                    break;
            }

            if (attenuate)
            {
                float targetVolume = Sounds.GetLinearRolloffAttenuation(_loopSource, volume);
                Sounds.SlideVolume(_loopSource, duration, targetVolume);
                Sounds.SlideLowPass(_loopSource, duration, cutoffFrequency);
            }
            else DisableOcclusion(duration);

            _muffled = attenuate;
        }

        private void DisableOcclusion(float duration)
        {
            float updatedVolume = Sounds.GetLinearRolloffAttenuation(_loopSource, _defaultVolume);
            Sounds.SlideVolume(_loopSource, duration, updatedVolume);
            Sounds.SlideLowPass(_loopSource, duration, 22000);
        }
        ...
        protected void PlayLoop()
        {
            ...
            _loopSource.maxDistance = _loopMaxDistance;
