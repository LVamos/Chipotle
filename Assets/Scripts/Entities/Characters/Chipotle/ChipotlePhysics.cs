...
namespace Game.Entities.Characters.Chipotle
{
	...
	{
		private void OnToggleSonar(ToggleSonar message)
		{
			SonarEnabled = !SonarEnabled;
			InnerMessage(new SonarToggled(this, SonarEnabled));
		}
		...
		protected override void HandleMessage(Message message)
		{
			switch (message)
			{
				case ToggleSonar m: OnToggleSonar(m); break;
	...
	.Select(group => group.First())
		...
		{
			Rectangle doorArea = door.Area.Value;
			Vector2? doorPoint = doorArea.GetAlignedPoint(Center);
			if (doorPoint == null)
				return false;

			Rectangle defaultPosition = Rectangle.FromCenter(doorPoint.Value, _area.Value.Height, _area.Value.Width);
			...
			foreach (Vector2 direction in lateralDirections)
			{
				...
				do
				{
					...
					MakeStep();
					return true;
				} while (doorArea.Intersects(newPosition));
			}
			return false;
